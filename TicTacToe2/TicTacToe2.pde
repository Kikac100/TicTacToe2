...
void draw(){
  ...
  //-----dodano(sat)
  textSize(18);
  if(pvp == 0) text("Igrate protiv compjutera", 800, height - 200);
  vrme = int(time/1000);
  
  text("Vrijeme potrošeno na prijašnji potez: " + vrme+ " s", 810, height-50);
  text("Ukupno vrijeme x: " + uko + " s", 810, height-300);
  text("Ukupno vrijeme o: " + ukx + " s", 810, height-470);
  ...
  if(label != ' '){
    ...
    text("Napomena: Tipkom b se pali/gasi computer player. ", 50, 4* height/5+100);
    strokeWeight (8);
  }
  else if (info ==1){
    background(100);
  ...
  if(stat > 0){
    ...
    textSize(30);
    textAlign(LEFT);
    text("na pocetku igre pali bot protivnika ",150, 350);
    fill(255);
    textAlign(CENTER);
    ...
    out.close();
      ...
      e.printStackTrace();
            ...
            void mousePressed(){
             ...
             if(coY>0 && coY<_width/3){stup = 0; mala += "A";ispo = 0;}
             else if(coY>_width/3 && coY<2*_width/3){stup = 1; mala += "B";ispo = 0;}
             else if(coY<_width) {stup=2; mala += "C";ispo = 0;}
             else {ispo = 1;}
            
             if(coX>0 && coX<height/3){red = 0; mala += "0";ispo = 0;}
             else if(coX>height/3 && coX<2*height/3){red = 1; mala += "1";ispo = 0;}
             else if(coX<height){red = 2; mala += "2";ispo = 0;}
             else {ispo = 1;}
             ...
             switch (mala){
              ...
              if (player=='x'  && ispo == 0 && Mala[malastup][malared]==' ' && IsLegal(red, stup)){
              ...
              else if(player=='o' && ispo == 0 && Mala[malastup][malared]==' ' && IsLegal(red, stup)){
                ...
                case "C2":
                  ...
                  break;
                     ...
                     while (randd>=0){
                         if(arr[korak] == ' ') randd--;
                         korak++;
                         if(korak == 8 && randd == rand) break;
                         if (korak>8) {korak = 0;}
                       }
                       if(arr[korak]!=' '&&korak!=0)korak--;
                       malared = korak%3;
                       //korak-=3;
                       while(korak>2){
                         korak-=3;
